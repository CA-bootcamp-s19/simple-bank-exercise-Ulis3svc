...
contract SimpleBank {
    ...
    function() external payable {
        revert("Function called with invalid data");
    }
    ...
    function enroll() public returns (bool){
        ...
        require(enrolled[customer] == false,"");
    ...
    function deposit() public payable returns (uint _balance) {
    ...
    function withdraw(uint withdrawAmount) public returns (uint) {
        ...
        uint wA = withdrawAmount;
        uint balance = balances[msg.sender];
        if(balance >= wA){
            balances[msg.sender] -= wA;
            balance = balances[msg.sender];
        }else{
            revert("Something happened");
        }
        (bool success,bytes memory returnedData) = msg.sender.call.value(wA)("");
        if (success){
            emit LogWithdrawal(msg.sender,wA,balances[msg.sender]);
        }
        return balance;
    }
